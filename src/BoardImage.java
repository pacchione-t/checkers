...
public class BoardImage {
    ...
    private WorldImage whitePiece = new CircleImage(SIZE * 5 / 12, OutlineMode.SOLID, Color.RED)
    ...
    private WorldImage whiteKing = new OverlayImage(
            new OverlayImage(new OverlayImage(new OverlayImage(new OverlayImage(new OverlayImage(
    ...
    public BoardImage() {
        WorldImage checkerboard = new RectangleImage(SIZE * 8, SIZE * 8, OutlineMode.SOLID, Color.WHITE)
                .movePinhole(-SIZE * 3, -SIZE * 4);

        WorldImage blackSquare = new RectangleImage(SIZE, SIZE, OutlineMode.SOLID, DARK_GREEN)
                .movePinhole(-SIZE / 2, -SIZE / 2);
    ...
    public WorldImage drawBoard(Checkerboard board, int selected) {
        ...
        return image.movePinhole(-SIZE, -SIZE * 8);
    }

    // Draws an individual square on the board
    private WorldImage drawSquare(int state, boolean selected) {
        return selected ? new OverlayImage(selectedSquare, drawPiece(state)) : drawPiece(state);
    }

    // Draws an individual piece (or empty square)
    private WorldImage drawPiece(int state) {
        switch (state) {
    ...
    public WorldImage drawPanel(boolean turn, int whitePieces, int blackPieces, boolean gameOver, boolean winner) {
        WorldImage panel = new RectangleImage(SIZE * 8, SIZE * 2, OutlineMode.SOLID, Color.WHITE);

        if (!gameOver) {
            panel = new OverlayOffsetImage(new TextImage("Turn:", SIZE, Color.BLACK), SIZE * 2.5, 0, panel);

            if (turn) {
                panel = new OverlayOffsetImage(
                        new CircleImage(SIZE / 2, OutlineMode.SOLID, Color.RED), SIZE / 2, 0, panel);
            } else {
                panel = new OverlayOffsetImage(
                        new CircleImage(SIZE / 2, OutlineMode.SOLID, Color.BLACK), SIZE / 2, 0, panel);
            }

            panel = new OverlayOffsetAlign(AlignModeX.LEFT, AlignModeY.MIDDLE,
                    new TextImage("Red: " + whitePieces, SIZE * 3 / 4, Color.BLACK), -SIZE * 4.5, -SIZE / 2.5, panel);

            panel = new OverlayOffsetAlign(AlignModeX.LEFT, AlignModeY.MIDDLE,
                    new TextImage("Black: " + blackPieces, SIZE * 3 / 4, Color.BLACK), -SIZE * 4.5, SIZE / 2.5, panel);
        } else {
            if (winner) {
                panel = new OverlayImage(new TextImage("Red Wins!", SIZE * 1.2, Color.BLACK), panel);
            } else {
                panel = new OverlayImage(new TextImage("Black Wins!", SIZE * 1.2, Color.BLACK), panel);
            }
        }
